...
import CalendarExportButton from "@/components/Button/CalendarExport";
...
export default function Calendar() {
  ...
  const offset = Math.max(0, WEEKDAY_EN.indexOf(firstWeekdayEn));
  const paddedDays = [...Array.from({ length: offset }, () => null), ...days];
    ...
    <div className="space-y-4">
      <div className="bg-white dark:bg-zinc-900 rounded-xl shadow p-6">
        ...
        <div className="grid grid-cols-7 gap-2">
          {paddedDays.map((d, idx) => {
            if (!d) {
              return <div key={`empty-${idx}`} className="p-2 rounded-lg" />;
            }
            ...
            return (
              <button
                ...
                className={`relative min-h-18 p-2 rounded-lg border text-left hover:bg-zinc-100 dark:hover:bg-zinc-700 w-full
      ...
      <div className="flex items-center justify-end mr-0">
        <CalendarExportButton
          ...
          hMonth={hMonth}
          hYear={hYear}
          monthLabel={HIJRI_MONTHS[hMonth - 1]}
          filename={`kalender-hijriyah-${HIJRI_MONTHS[hMonth - 1]}-${hYear}`}
        />
      </div>
    </div>
  );
